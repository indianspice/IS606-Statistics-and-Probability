---
title: "Probability and Statistics LAB2"
...
```{r help-sample, eval=TRUE,tidy = TRUE}
?sample
```
...
```{r}
play_streak <- calc_streak(sim_basket)
barplot(table(play_streak))
```
...
The distribution is skewed to the right. The typical streak length of this simulated independent shooter is zero. The longest streak is 8.
...
If I ran the simulation of the indepent shooter a second time the distrubtion would be similar to the first with some variations. The probality for the independent shooter is the same in both simulations so the outcomes will be similiar. 
...
Eventhough, both Kobe and the simulated player had more zero streaks as compared to non-zero streaks, Kobe's streak pattern seems to fit the hot hand model more so than the simulated player. There could also be some confounding variables at play.
