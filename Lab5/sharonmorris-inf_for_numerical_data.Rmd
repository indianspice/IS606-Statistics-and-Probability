---
title: "Inference for numerical data -- IS606 Lab5"
...
The case in the data set are parents of babies born in North Carolina. There are 1000 cases in our sample
...
Catetorical variables: mature, maritial, gender, habit, premie, lowbirthweight, momwhite
Numerical variables: fage, mage, weeks, visits, gained, weight, 

Yes there are outliers for numerical variables
...
```{r}
library(ggplot2)
hw <- ggplot(data=nc) + geom_boxplot(aes(x=habit, y=weight))
hw
```
...
```{r}
hw <- ggplot(data=nc) + geom_bar(aes(x=weight)) + facet_grid(habit ~ .) 
hw
```
...
H_0 = There is not difference between the average weights of babies born to smoking and non-smoking mothers
H_A = There is a difference between the average weights of babies born to smoking and non-smoking mothers
...
```{r inf-weight-habit-ci, eval=TRUE, tidy=TRUE}
inference(y = nc$weight, x = nc$habit, est = "mean", type = "ci", null = 0, 
          alternative = "twosided", method = "theoretical", 
...
We accept the alternative hypothesis -- the mother's maturity has an affect on the baby's weight
...
The maximum age of younger moms 34, the age of mature moms is 35. The cutoff of younger moms is 34 or younger. 
...
H_O = There is no difference between the number of visit to the doctor of married and unmarried moms
H_A = There is a difference between the number of visit to the doctor of married and unmarried moms

With a pvalue = 0, the null hypothesis is rejected -- there is a difference between the number of visists to the doctor of married and unmarried women
