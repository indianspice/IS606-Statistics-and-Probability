---
title: "Introduction to linear regression IS607 Lab 7"
...
examining the linear relationship between runs scored in a season and a number of other player statistics. Our aim will be to summarize these relationships both graphically and numerically in order to find which variable, if any, helps us best predict a team's runs scored in a season.
...
There is a positive relationship between at-bats and runs. There are some outlyers.

```{r}
library(ggplot2)
g1 <- ggplot(data=mlb11, aes(x=at_bats, y=runs)) + geom_point() 
g1
```
...
There is a medium strength positve relationship between runs and at_bats. As the runs increase so does at_bats. There appear to be greater variability around the lover end of the line
...
The smallest sum of squares: 130841.1
...
The slope of r m2$coefficients[2] tells us there is a r m2$coefficients[2] run increase for every 1 homerun increase. 
...
The residual will be close to zero. It is an underestimation

```{r}
atBats <- 5578 
pRuns <- (-2789.2429 + 0.6305) * atBats
pRuns

mlb11[mlb11$at_bats == atBats]
```
...
There are more points below the line than above with a few outliners above. There doesn't appear to be a great deal of variability.
...
Yes, based on the histogram and the normal probability plot the nearly normal residuals condubiont appear to have been met.
...
8.  Based on the plot in (1), does the constant variability condition appear to 
    ...
    Yes, the constant variablility condition appear to have been met.
...
There appears to be a linear relationship with some outliners
...
There is a stronger correlation between runs and at_bats than there is between runs and stolen_bases -- at_bats seems to predict the variable getter than stolen_bases
...
bat_avg has a positive relationshipe with runs -- as runs increase so does bat_avg

```{r}
library(ggplot2)
gOther <- ggplot(data=mlb11, aes(x=bat_avg, y=runs)) + geom_point() 
gOther
...
new_onbase seems to be the best predictor

```{r}
newBase <- lm(runs ~ new_onbase, data = mlb11)
newSlug <- lm(runs ~ new_slug, data = mlb11)
newObs <- lm(runs ~ new_obs, data = mlb11)
summary(newBase)
summary(newSlug)
summary(newObs)
```
...
```{r}
gNew1 <- ggplot(data=mlb11, aes(x=new_onbase, y=runs)) + geom_point()
gNew1
```

```{r}
hist(newObs$residuals)
```
